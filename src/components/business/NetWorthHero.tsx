  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap gap-1.5 mt-2.5 text-[11px] font-nunito font-bold">
        <span className={chip}>📦 Stock &amp; assets {formatCash(businessWorthCents, currency)}</span>
        <span className={chip}>🐝 Hive {formatCash(hiveWorthCents, currency)}</span>
        <span className={chip}>📈 Invested {formatCash(investedCents, currency)}</span>
      </div>
    </div>
  );
}
