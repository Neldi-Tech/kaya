...
//
// Layout: phone-first (the approved design). On a laptop the frame opens
// into two columns — the shelves + the Bookworm Wall on the left, the
// "what needs someone" rail (Game Night · dust · missing · lent · names
// to confirm · settings) on the right.
...
export default function CupboardHomePage() {
  ...
  const dust = shelf ? dustItems(live, shelf.settings.dustDays, today).slice(0, 3) : [];
  const stats = shelf ? cupboardWeekStats(live, today) : null;
  const wallHasSomething = !!stats && (stats.readingNow.length > 0 || stats.pagesThisWeek > 0 || stats.finishedThisMonth.length > 0 || stats.playedThisWeek.length > 0);

  const sub = shelf === null
    ? 'Loading…'
    : `Ours, together · 📚 ${liveBooks.length} book${liveBooks.length === 1 ? '' : 's'} · 🎲 ${liveGames.length} game${liveGames.length === 1 ? '' : 's'}`;

  // ── the right rail (desktop) / the tail (phone) ──
  const aside = shelf && (
    <>
      {liveGames.length > 0 && shelf.settings.gameNight.enabled && (
        <div className="rounded-[13px] border border-[#EFD9A0] bg-[#FFF1C9] p-3 mb-2.5">
          ...
          <div className="mt-2"><Pill bg="#D4A847" fg="#3D2E08" onClick={() => setPicker(true)}>🎡 Pick tonight&rsquo;s game</Pill></div>
        </div>
      )}
      ...
      <Card>
        <div className="font-display font-extrabold text-[11px] tracking-[1.2px] text-[#5A6488] uppercase mb-2">Quick doors</div>
        ...
        {ended.length > 0 && (
          <p className="text-[10.5px] text-[#8A8471] leading-snug mt-3 mb-0">
  ...
  return (
    <>
      <CupboardFrame
        back={back}
        hero={{ tone: 'wood', eyebrow: 'Sparks › Treasures', title: '🗄 The Family Cupboard', sub }}
        aside={aside || undefined}
        actions={shelf ? (
          <>
            <Pill bg="#fff" fg={WOOD_DK} onClick={() => setScanning('book')}>📷 Scan to add</Pill>
            <Pill bg="rgba(255,255,255,.18)" fg="#fff" href="/sparks/treasures/cupboard/books">📚 Books</Pill>
            <Pill bg="rgba(255,255,255,.18)" fg="#fff" href="/sparks/treasures/cupboard/games">🎲 Games</Pill>
          </>
        ) : undefined}
      >
        ...
        {shelf && (
          <>
            {/* phone: the primary doors (on desktop they sit in the hero) */}
            ...
            {live.length === 0 && (
              <Card tone="wood" className="lg:p-8">
                <div className="text-[30px] lg:text-[44px] leading-none text-center">🗄</div>
                <p className="text-[13px] lg:text-[16px] font-extrabold text-[#0F1F44] mt-1.5 mb-0 text-center">The shelves are empty</p>
                <p className="text-[11.5px] lg:text-[13px] text-[#5A6488] mt-1.5 leading-snug text-center max-w-md mx-auto">
                  Add the books and games the family shares. Anyone can add — point the camera at a barcode, or snap the front, and the name comes from the scan.
                </p>
            ...
            {/* 📚 🎲 the two shelves */}
            {live.length > 0 && (
              <div className="grid grid-cols-2 gap-2.5 lg:gap-4 mb-2.5 lg:mb-4">
                <Link href="/sparks/treasures/cupboard/books" className="rounded-[14px] lg:rounded-[18px] border border-[#E4CDB2] bg-[#F6ECDF] p-3 lg:p-5 no-underline hover:border-[#8B5E34] transition-colors">
                  <div className="text-[22px] lg:text-[32px] leading-none">📚</div>
                  <div className="font-display font-extrabold text-[13px] lg:text-[17px] text-[#0F1F44] mt-1.5">Book Shelf</div>
                  <div className="text-[10.8px] lg:text-[12px] font-bold text-[#5B6B8C] mt-0.5 leading-snug">
                    {liveBooks.length} book{liveBooks.length === 1 ? '' : 's'} · {famCount(liveBooks)} family · {liveBooks.length - famCount(liveBooks)} kids&rsquo;
                    {stats && stats.readingNow.length ? ` · ${stats.readingNow.length} reading now` : ''}
                  </div>
                </Link>
                <Link href="/sparks/treasures/cupboard/games" className="rounded-[14px] lg:rounded-[18px] border border-[#E4CDB2] bg-[#F6ECDF] p-3 lg:p-5 no-underline hover:border-[#8B5E34] transition-colors">
                  <div className="text-[22px] lg:text-[32px] leading-none">🎲</div>
                  <div className="font-display font-extrabold text-[13px] lg:text-[17px] text-[#0F1F44] mt-1.5">Game Shelf</div>
                  <div className="text-[10.8px] lg:text-[12px] font-bold text-[#5B6B8C] mt-0.5 leading-snug">
                    {liveGames.length} game{liveGames.length === 1 ? '' : 's'} · {famCount(liveGames)} family · {liveGames.length - famCount(liveGames)} kids&rsquo;
                    {stats && stats.playedThisWeek.length ? ` · ${stats.playedThisWeek.reduce((n, p) => n + p.times, 0)} played this week` : ''}
                  </div>
                </Link>
              </div>
            )}
            ...
            {stats && wallHasSomething && (
              <Card tone="good" className="lg:p-4">
                <div className="font-display font-extrabold text-[12.5px] lg:text-[14px] text-[#0E6B5E]">🐛 Bookworm Wall</div>
                <div className="lg:grid lg:grid-cols-2 lg:gap-x-6">
                  {stats.readingNow.slice(0, 6).map((r) => {
                    ...
                    return (
                      <Link key={`${r.treasureId}-${r.readerKidId}-${r.readerName}`} href={`/sparks/treasures/cupboard/${r.treasureId}`} className="block mt-1.5 no-underline">
                        <div className="text-[11px] lg:text-[12px] font-bold text-[#2C4A44]">
                ...
                <p className="text-[10.8px] lg:text-[12px] font-bold text-[#2C4A44] mt-2 m-0 leading-snug">
                  This week <b>{stats.pagesThisWeek} page{stats.pagesThisWeek === 1 ? '' : 's'}</b>
                  {stats.byReader.length ? ` (${stats.byReader.map((b) => `${b.name.split(' ')[0]} ${b.pages}`).join(' · ')})` : ''}
                  {' · '}this month <b>{stats.finishedThisMonth.length} book{stats.finishedThisMonth.length === 1 ? '' : 's'} finished</b>
                  {stats.playedThisWeek.length ? ` · 🎲 ${stats.playedThisWeek.reduce((n, p) => n + p.times, 0)} game${stats.playedThisWeek.reduce((n, p) => n + p.times, 0) === 1 ? '' : 's'} this week` : ''}
