...
export default function DriversHomePage() {
  ...
  useEffect(() => {
    ...
    void Promise.all(scheduled.map(async (v) => {
      ...
      const due = computeServiceDue({
        ...
        nextKmOverride: v.nextServiceKm,
        nextDateOverride: v.nextServiceDate,
  ...
  // Web-Fit (2026-08-23): wide tier, main + rail. Desktop: "＋ New driver
  // request" lives in the header; the CTA/templates block + vehicle/kind
  // pickers sit in the right rail; request piles render as dense rows.
  ...
  return (
    <Page width="wide">
      ...
      {vehicles.length > 0 && (
        <div className="flex gap-2 overflow-x-auto pb-1 mb-3 -mx-4 px-4 snap-x [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden lg:flex-wrap lg:overflow-visible lg:mx-0 lg:px-0">
          {vehicles.map((v) => {
