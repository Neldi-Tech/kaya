...
export default function GoalsPage() {
  ...
  // Web-Fit (2026-08-23): content tier. Desktop: "+ Set a new goal" lives
  // in the header; goal cards flow 2-up. Mobile markup/order unchanged.
  const newGoalAction = (
    <Link
      href="/hive/goals/new"
      className="inline-flex items-center h-10 px-5 rounded-hive bg-hive-honey hover:bg-hive-honey-dk text-white font-nunito font-black text-sm no-underline shadow-[0_8px_20px_-8px_rgba(243,156,47,0.5)] transition-colors"
    >
      + Set a new goal
    </Link>
  );
  ...
  return (
    <Page width="content">
      ...
      <PageHeader actions={newGoalAction}>
      ...
      <div className="space-y-3 lg:space-y-0 lg:grid lg:grid-cols-2 lg:gap-3 lg:items-start">
        ...
        <Link
          href="/hive/goals/new"
          className={`block rounded-hive-lg border-2 border-dashed border-hive-line p-5 text-center font-nunito font-extrabold text-[13px] text-hive-muted hover:border-hive-honey hover:text-hive-honey-dk transition-colors no-underline ${active.length > 0 ? 'lg:hidden' : ''}`}
        >
          + Set a new goal
        </Link>
      </div>
      ...
      {done.length > 0 && (
        <>
          ...
          <div className="space-y-3 opacity-70 lg:space-y-0 lg:grid lg:grid-cols-2 lg:gap-3 lg:items-start">
            {done.map((g) => (
              <GoalCard
                key={g.id}
                goal={g}
