// GET  /api/buzz/[id]/comments — list (sanitised).
// POST /api/buzz/[id]/comments — add a comment.

import { NextRequest, NextResponse } from 'next/server';
import { FieldValue } from 'firebase-admin/firestore';
import { resolveAuth, sanitizeComment, trimToLen, loadBuzzSettings, type RawComment } from '@/lib/buzzServer';
...
export async function GET(req: NextRequest, ctx: { params: { id: string } }) {
  ...
  const snap = await db.collection('buzz').doc(id).collection('comments').orderBy('createdAt', 'asc').limit(200).get();
...
export async function POST(req: NextRequest, ctx: { params: { id: string } }) {
  ...
  if (!buzzSnap.exists) return NextResponse.json({ error: 'buzz-not-found' }, { status: 404 });

  const commentRef = buzzRef.collection('comments').doc();
